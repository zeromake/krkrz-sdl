	...
	tTVPGraphicLoadMode mode)
{
	...
	auto datasize = src->GetSize();
	std::unique_ptr<uint8_t[]> data(new uint8_t[datasize]);
	auto read = src->Read(data.get(), datasize);
...
void _TVPLoadHeaderWEBP(void* formatdata, tTJSBinaryStream *src, iTJSDispatch2** dic) {
...
bool _TVPAcceptSaveAsWebP(void* formatdata, const ttstr & type, class iTJSDispatch2** dic ) {
	if( type.StartsWith(TJS_W("webp")) || (type == TJS_W(".webp")) ) return true;
	return false;
}

static int _WebPIStreamWrite(const uint8_t* data, size_t data_size, const WebPPicture* pic)
{
...
void _TVPSaveAsWebP(void* formatdata, void* callbackdata, IStream* dst, const ttstr & mode, tjs_uint width, tjs_uint height, tTVPGraphicSaveScanLineCallback scanlinecallback, iTJSDispatch2* meta )
{
	if (height == 0 || width == 0)
	{
