 ...
 *
 * date: 2016/11/01 author: T.Imoto;
 * TJS2 用に修正
 **************************************************************/
...
#define char_to_int(p) ((p)- TJS_W('0'))
#ifndef MAX
...
dopr(tjs_char *buffer, size_t maxlen, const tjs_char *format, va_list args_in)
{
	...
	while (state != DP_S_DONE) {
		...
		case DP_S_MOD:
			...
			case TJS_W('l'):
				cflags = DP_C_LONG;
				ch = *format++;
				if (ch == TJS_W('l')) {	/* It's a long long */
					cflags = DP_C_LLONG;
					ch = *format++;
				}
				break;
		...
		case DP_S_CONV:
			...
			case TJS_W('o'):
				...
				if (cflags == DP_C_SHORT)
					...
					value = (long)va_arg (args, tjs_uint64 );
				else if (cflags == DP_C_SIZE)
					value = va_arg (args, size_t);
#ifdef notyet
				else if (cflags == DP_C_INTMAX)
			...
			case TJS_W('u'):
				...
				if (cflags == DP_C_SHORT)
					...
					value = (LLONG)va_arg (args, tjs_uint64 );
				else if (cflags == DP_C_SIZE)
					value = va_arg (args, size_t);
#ifdef notyet
				else if (cflags == DP_C_INTMAX)
			...
			case TJS_W('s'):
				...
				if (!strvalue) (tjs_char*)TJS_W("(NULL)");
				if (max == -1) {
	...
	if (maxlen != 0) {
		if (currlen < maxlen - 1) 
			buffer[currlen] = TJS_W('\0');
		else if (maxlen > 0) 
			buffer[maxlen - 1] = TJS_W('\0');
	}
...
static int
fmtint(tjs_char *buffer, size_t *currlen, size_t maxlen,
    intmax_t value, int base, int min, int max, int flags)
{
	int signvalue = 0;
	tjs_uint64 uvalue;
	...
	do {
		convert[place++] =
			(caps? TJS_W("0123456789ABCDEF"):TJS_W("0123456789abcdef"))
...
namespace TJS
{
	...
	tjs_int TJS_snprintf( tjs_char *str, size_t count, const tjs_char *fmt, ... )
	{
		size_t ret;
		va_list ap;
