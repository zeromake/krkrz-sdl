...
#ifdef __cplusplus
#ifdef UNICODE
#define __tjs_isspace iswspace
#define __tjs_isdigit iswdigit
#define __tjs_isalpha iswalpha
#else
#define __tjs_isspace isspace
#define __tjs_isdigit isdigit
#define __tjs_isalpha isalpha
#endif

inline bool TJS_iswspace(tjs_char ch) {
	// the standard iswspace misses when non-zero page code
	if(ch&0xff00) return false; else return 0!=::__tjs_isspace(ch);
}
inline bool TJS_iswdigit(tjs_char ch) {
	// the standard iswdigit misses when non-zero page code
	if(ch&0xff00) return false; else return 0!=::__tjs_isdigit(ch);
}
inline bool TJS_iswalpha(tjs_char ch) {
	// the standard iswalpha misses when non-zero page code
	if(ch&0xff00) return true; else return 0!=::__tjs_isalpha(ch);
}
#endif
